...
export function StudyProvider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        await sched.init();
        await seedNewCards();
        const [g, r, c] = await Promise.all([game.load(), lessonsLib.readMap(), sched.counts()]);
        setGameState(g);
        setRead(r);
        setCounts(c);
        setReady(true);
  ...
  const markLesson = useCallback(async (id, wasRead) => {
    ...
    if (!wasRead) {
      ...
      toast(`Lesson read · +${r.xp} XP`, 'sage');
    } else {
  ...
  const value = useMemo(() => ({
    ...
    reload: async () => {
      const [g, r, c] = await Promise.all([game.load(), lessonsLib.readMap(), sched.counts()]);
      gameRef.current = g;
      setGameState(g); setRead(r); setCounts(c);
    },
  }), [ready, error, gameState, counts, read, toasts, burst, levelUp, seals,
       refreshCounts, markLesson, award, toast, celebrate, dismissLevelUp, dismissSeal, setDailyGoal]);
